...
charge_data=( [LI]=1.00000 [NA]=1.00000 [KA]=1.00000 [RB]=1.00000 [CS]=1.00000 [BE]=2.00000 [MG]=2.00000 [CA]=2.00000 [SR]=2.00000 [BA]=2.00000 [B]=3.00000 [AL]=3.00000 [GA]=3.00000 [FL]=-1.00000 [CL]=-1.00000 [BR]=-1.00000 [IO]=-1.00000 [NB]=0.70000  [NC]=-0.500000 [ND]=1.000000 [NE]=1.000000 [FB]=-0.700000 [FC]=-0.500000 [FD]=-1.000000 [FE]=-1.000000)
declare -A mass_data
mass_data=( [LI]=6.9400 [NA]=22.9898 [KA]=39.0983 [RB]=85.4678 [CS]=132.9055 [BE]=9.012182 [MG]=24.3050 [CA]=40.0780 [SR]=87.62 [BA]=137.327 [AL]=26.98154 [FL]=18.9984 [CL]=35.4500 [BR]=79.9040 [IO]=126.9045 [NB]=22.9898 [NC]=22.9898 [ND]=22.9898 [NE]=22.9898 [FB]=18.9984 [FC]=18.9984 [FD]=18.9984 [FE]=18.9984 )
...
#if [ $(ls "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_[0-9]/Production_MD"$forback"/md*.xvg "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_[0-9][0-9]/Production_MD"$forback"/md*.xvg | wc -l) -ge 21 ]
#then
#cd "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"
...
end_steps=$( tail -n300 "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_"$i"/Production_MD"$forback"/md"$i".log | grep 'Statistics' | awk '{print $3}')
actual_length=$(echo "$end_steps * 2 / 1000 / 1000" | bc )
echo $1 $2 $3 $4 $5 $6"paired" $actual_length $7 $8 $9 $FE $FEerr $SA $SAerr $SB $SBerr $CN $CN_err $MRT $MRT_err $MRT_fit $MRT_fit_err $IP $IPL1 $IPL2
cd $wd
exit 0
fi
exit 0
...
#qued=`qstat -f | tr -d '\n' | tr -d '[:blank:]' | grep "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"  | wc -l`
qued=`qstat -f | tr -d '\n' | tr -d '[:blank:]' | grep "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/"$cation""$anion"."$forback".olog  | wc -l`
...
if grep 'Finished mdrun' "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_20/Production_MD"$forback"/md20.log   && [ $force != 'yes' ]
then
echo skip to analysis
else

echo cp template_solvating.top "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
cp template_solvating.top "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
...
#if [ ! -f  "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/"$cation""$anion"_water_fixed.gro ] || ( grep 'Finished mdrun' "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_*/Production_MD/m*.log )
#if ! grep 'Finished mdrun' "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_*/Production_MD"$forback"/m*.log 
#then
cp /scratch/g15/kpg575/GROMACS/empty-box.gro empty-box.gro
...
echo "Solvating"
gmx solvate -cp empty-box.gro -o water-box.gro -cs spc216.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top #fix watermodel in this line
#exit 0
mv water-box.gro "$cation""$anion"_"$conc"M_water.gro
gmx grompp -f minim.mdp -c "$cation""$anion"_"$conc"M_water.gro -o ion.tpr -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -maxwarn 2
#cp topol-water.top topol-"$cation""$anion"-"$conc"M.top
echo number of ions is $nions at "$conc"M for box with width "$box_size"nm
#if [ ! -f  "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/"$cation""$anion"_water_fixed.gro ] || ( grep 'Finished mdrun' "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_*/Production_MD/m*.log )
#if  ( grep 'Finished mdrun' "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"/Lambda_*/Production_MD/m*.log )
#then
	if  [ $cation_or_anion == 'salt' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -pname $cation2 -nname $anion2 -np $nions2 -nn $nions2 -rmin 0.3
		if [ ! $conc == 0 ]
		then
	        gmx grompp -f minim.mdp -c "$cation""$anion"_"$conc"M_water.gro -o ion2.tpr -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -maxwarn 2
		echo "2" | gmx genion -s ion2.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -pname $cation2 -nname $anion2 -np 1 -nn 1 -rmin 0.3
		fi
	elif [ $cation_or_anion == 'cation' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -pname $cation2 -nname $anion2 -np $nions -nn $nions2 -rmin 0.3
	elif [ $cation_or_anion == 'anion' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -pname $cation2 -nname $anion2 -np $nions2 -nn $nions -rmin 0.3
	elif [ $cation_or_anion == 'water' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top -pname $cation2 -nname $anion2 -np $nions2 -nn $nions2 -rmin 0.3
	fi
...
if [ ! -d "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep" ]
then
mkdir "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"
fi

fi # end of check - to prevent making new box; will jump to analysis


fewd=$( echo "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep" )
#cd "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"
#mv $wd/"$cation""$anion"_"$conc"M_water.gro "$cation""$anion"_water_fixed.gro
cp -r $wd/template_files/MDP $fewd/.
cp $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top $fewd/topol.top
echo XXXXXXXXXXXXXXXXXXX mv $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top $fewd/topol.top XXXXXXXXXXXXXXXXXXXXXX
...
#sed -i 's/NA /'$cation'/g' "$cation""$anion"_water_fixed.gro
	if [ $conc  == 0 ]
	then
	sed -i 's/'$cation'               1/'$cation'D               1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	# Reverse the file, replace the first occurrence (which is actually the last in the original file), and reverse it back
	tac $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top | awk '!changed && /"\$anion"/{sub(/"\$cation"/, "\"$cation\"D"); changed=1} 1' | tac > temp.txt
	
	# Move the temporary file back to the original file
	mv temp.txt $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	else
	sed -i 's/'$cation'               '$nions'/'$cation'               '$nions2'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	echo ''$cation'D               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	fi
elif [ $cation_or_anion == 'anion' ]
then
...
#sed -i 's/NA /'$anion'/g' "$cation""$anion"_water_fixed.gro
	if [ $conc == 0 ]
	then
	sed -i 's/'$anion'               1/'$anion'D               1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	# Reverse the file, replace the first occurrence (which is actually the last in the original file), and reverse it back
        tac $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top | awk '!changed && /"\$anion"/{sub(/"\$anion"/, "\"$anion\"D"); changed=1} 1' | tac > temp.txt

        # Move the temporary file back to the original file
        mv temp.txt $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	else
	sed -i 's/'$anion'               '$nions'/'$anion'               '$nions2'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	echo ''$anion'D               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	fi
...
#
#line_insert_num=$(grep -n '; Define LI molecule type' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top | awk -F':' '{print $1}')
#line_insert_num2=$(echo "$line_insert_num - 3" | bc )
#
#printf '%s\n' "$multilinevariable" | sed -i ''$line_insert_num2'r /dev/stdin' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top

sed -i 's/NA in water/'$ion' in water/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
sed -i 's/NA                       1/'$ion'                       1/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
...
#	sed -i 's/'$anion'               1/'$anion'D               1/' $fewd/topol.top
	sed -i 's/'$cation'               1/SAL               1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	sed -i '/'$anion'               1/d' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
        else
        sed -i 's/'$cation'               '$nions'/'$cation'               '$nions3'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	sed -i 's/'$anion'               '$nions'/'$anion'               '$nions3'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
        sed -i '0,/'$cation'               1/{//d}' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
        sed -i '0,/'$anion'               1/{//d}' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	if [ -f $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top ]
	then
	echo 'SAL               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
	fi
...
nsol=$(grep 'SOL' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top | tail -n1 | awk '{print $NF}')
lowsol=$( echo "$nsol -1" | bc )

echo nsol $nsol low sol $lowsol
#sed -i 's/ '$nsol'/'$lowsol'/
# DIS    1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
sed -i 's/'$nsol'SOL/'$nsol'DIS/g' $wd/"$cation""$anion"_"$conc"M_water.gro
sed -i 's/SOL         '$nsol'/SOL         '$nsol'/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
...
sed -i 's/'$nsol'/'$lowsol'/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
#sed -i "s/${nsol}/$(echo -e "${lowsol}\nDIS 1")/g" $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
echo 'DIS               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
fi
cd $fewd
...
#then
	mv $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top topol.top
...
#fi
#rm $wd/"$cation""$anion"_"$conc"M_water.gro
#rm $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$boxsize""$rep".top
...
then
if ( [ $cation == 'NA' ] && [ $anion == 'FL' ] ) || ( [ $cation == 'NB' ] && [ $anion == 'FB' ] ) || ( [ $cation == 'NC' ] && [ $anion == 'FC' ] )  || ( [ $cation == 'ND' ] && [ $anion == 'FD' ] ) || ( [ $cation == 'NE' ] && [ $anion == 'FE' ] ) ; then
#shortx=0.28
#longx=0.34
shortx=0.23
longx=0.28
elif  [ $cation == 'NA' ] && [ $anion == 'CL' ] ; then
#shortx=0.35
#longx=0.4
shortx=0.285
longx=0.375
elif [ $cation == 'NA' ] && [ $anion == 'IO' ] ; then
#shortx=0.41
#longx=0.47
shortx=0.33
longx=0.45
...
echo $cation $anion
echo ensure bond lengths selected well for the ion pair.
exit 0
fi
#bias=500
bias=10000

if [ -s ${cationmass} ] ||[ -s ${cationcharge} ] 
then
echo cationcharge is empty?  ${cationcharge} OR cationmass is empty? ${cationmass}
exit 0
fi
...
#if [ ! -f bar.xvg ]
#then
#qued=`qstat -f | tr -d '\n' | tr -d '[:blank:]' | grep "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$boxsize""$rep"  | wc -l`
#if [ $qued -ge 1 ]
#then
#echo Not queuing
#else
...
cp $wd/template_files/qjob_parallel $fewd/.
sed -i 's/NA/'$cation''$anion'/g' ./qjob_parallel
