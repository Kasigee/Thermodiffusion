...
if [ $force != 'yes' ]
then
if [ $(ls "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"/Lambda_[0-9]/Production_MD"$forback"/md*.xvg "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"/Lambda_[0-9][0-9]/Production_MD"$forback"/md*.xvg | wc -l) -ge 21 ]
then
cd "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep" 
end_steps=$( tail -n300 "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"/Lambda_10/Production_MD"$forback"/md*.log | grep 'Statistics' | awk '{print $3}')
...
qued=`qstat -f | tr -d '\n' | tr -d '[:blank:]' | grep "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"  | wc -l`
...
cp ../qjob_extension_parallel_template qjob_extension
fi
...
./qjob_extension $forback
exit 0
if grep 'Finished mdrun' "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"/Lambda_20/Production_MD/md20.log   && [ $force != 'yes' ]
then
echo skip to analysis
else
cp template_solvating.top "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
...
if [ ! -f  "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep""$cation""$anion"_water_fixed.gro ] || ( grep 'Finished mdrun' "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"/Lambda_*/Production_MD/m*.log )
then
	if  [ $cation_or_anion == 'salt' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top -pname $cation2 -nname $anion2 -np $nions2 -nn $nions2 -rmin 0.3
		if [ ! $conc == 0 ]
		then
	        gmx grompp -f minim.mdp -c "$cation""$anion"_"$conc"M_water.gro -o ion2.tpr -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top -maxwarn 2
		echo "2" | gmx genion -s ion2.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top -pname $cation2 -nname $anion2 -np 1 -nn 1 -rmin 0.3
		fi
	elif [ $cation_or_anion == 'cation' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top -pname $cation2 -nname $anion2 -np $nions -nn $nions2 -rmin 0.3
	elif [ $cation_or_anion == 'anion' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top -pname $cation2 -nname $anion2 -np $nions2 -nn $nions -rmin 0.3
	elif [ $cation_or_anion == 'water' ]
	then
	echo "2" | gmx genion -s ion.tpr -o "$cation""$anion"_"$conc"M_water.gro -p "$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top -pname $cation2 -nname $anion2 -np $nions2 -nn $nions2 -rmin 0.3
	fi
...
then
mkdir "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"
fi

fi # end of check - to prevent making new box; will jump to analysis


fewd=$( echo "$wd"/"$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep" )
#cd "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"
#mv $wd/"$cation""$anion"_"$conc"M_water.gro "$cation""$anion"_water_fixed.gro
cp -r $wd/template_files/MDP $fewd/.
#mv $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top topol.top
if [ $parallel == 'no' ]
then
...
#sed -i 's/NA /'$cation'/g' "$cation""$anion"_water_fixed.gro
	if [ $conc  == 0 ]
	then
	sed -i 's/'$cation'               1/'$cation'D               1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	else
	sed -i 's/'$cation'               '$nions'/'$cation'               '$nions2'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	echo ''$cation'D               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	fi
elif [ $cation_or_anion == 'anion' ]
then
...
#sed -i 's/NA /'$anion'/g' "$cation""$anion"_water_fixed.gro
	if [ $conc == 0 ]
	then
	sed -i 's/'$anion'               1/'$anion'D               1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	else
	sed -i 's/'$anion'               '$nions'/'$anion'               '$nions2'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	echo ''$anion'D               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	fi
...
#
#line_insert_num=$(grep -n '; Define LI molecule type' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top | awk -F':' '{print $1}')
#line_insert_num2=$(echo "$line_insert_num - 3" | bc )
#
#printf '%s\n' "$multilinevariable" | sed -i ''$line_insert_num2'r /dev/stdin' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top

sed -i 's/NA in water/'$ion' in water/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
sed -i 's/NA                       1/'$ion'                       1/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
...
#	sed -i 's/'$anion'               1/'$anion'D               1/' $fewd/topol.top
	sed -i 's/'$cation'               1/SAL               1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	sed -i '/'$anion'               1/d' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
        else
        sed -i 's/'$cation'               '$nions'/'$cation'               '$nions3'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	sed -i 's/'$anion'               '$nions'/'$anion'               '$nions3'/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
        sed -i '0,/'$cation'               1/{//d}' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
        sed -i '0,/'$anion'               1/{//d}' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	if [ -f $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top ]
	then
	echo 'SAL               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
	fi
...
nsol=$(grep 'SOL' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top | tail -n1 | awk '{print $NF}')
lowsol=$( echo "$nsol -1" | bc )

echo nsol $nsol low sol $lowsol
#sed -i 's/ '$nsol'/'$lowsol'/
# DIS    1/' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
sed -i 's/'$nsol'SOL/'$nsol'DIS/g' $wd/"$cation""$anion"_"$conc"M_water.gro
sed -i 's/SOL         '$nsol'/SOL         '$nsol'/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
...
sed -i 's/'$nsol'/'$lowsol'/g' $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
#sed -i "s/${nsol}/$(echo -e "${lowsol}\nDIS 1")/g" $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
echo 'DIS               1' >> $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
fi
cd $fewd
...
rm $wd/"$cation""$anion"_"$conc"M_water_"$cation_or_anion"paired"$rep".top
...
#if [ ! -f bar.xvg ]
#then
#qued=`qstat -f | tr -d '\n' | tr -d '[:blank:]' | grep "$cation""$anion"_"$conc"M_"$temperature"K_"$length"ns_"$cation_or_anion"paired"$rep"  | wc -l`
#if [ $qued -ge 1 ]
#then
#echo Not queuing
#else
